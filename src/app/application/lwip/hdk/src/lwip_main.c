...
// // waf clean_bin        1           /** EVM/Hardware dependent & is same for TMS570 & RM48 HDKs */
extern uint8 emacAddress[6U] = {0x00U, 0x08U, 0xEEU, 0x03U, 0xA6U, 0x6CU};
extern uint32 emacPhyAddress = 0U;
...
void iommMuxEnableMii(void) {
    ...
    *(int *)0xFFFFEB44 |=
        ((1 << 26) | (1 << 18) | (1 << 10) | (1 << 2));  //P13[2, 10, 26,18]   //Mux 13 Tx[2],TxEn,Tx[1],Tx[0]
...
void sciDisplayText(sciBASE_t *sci, uint8_t *text, uint32_t length) {
    ...
    for (i = 0; i < length - 1; i++) {
        ;                          /* wait until busy */
        sciSendByte(sci, text[i]); /* send out text   */
    };
}
